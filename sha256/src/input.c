...
void mov512(uint8_t *in, uint8_t *target) {
  for (int i = 0; i < 64; i++) {
    target[i] = in[i];
  }
}
...
int main(int argc, char **argv) {
  ...
  uint8_t *text = malloc(64 * times * sizeof(uint8_t));
  uint8_t *result_cpu = malloc(32 * times * sizeof(uint8_t));
  uint8_t *result_fpga = malloc(32 * times * sizeof(uint8_t));

#pragma omp target map(to                                                      \
                       : text [0:(64 * times)])                                \
    map(tofrom                                                                 \
        : result_fpga [0:(32 * times)]) map(to                                 \
                                            : times)
  { sha_series(text, result_fpga, times); }

  sha_series(text, result_cpu, times);

  for (int i = 0; i < times; i++) {
    if (result_cpu[i] != result_fpga[i]) {
      fprintf(stderr, "OUTPUT MISMATCH AT INDEX %d!\n", i);
      exit(EXIT_FAILURE);
    }
  }
